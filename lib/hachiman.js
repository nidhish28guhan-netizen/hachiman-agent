...
import { makeMcpAdapter } from '../packages/adapters/src/mcp.js';
  ...
  const node = {
    ...
    status() {
      return {
        ...
        adapters: node.adapters ? node.adapters.list().map((a) => a.id) : [],
      };
    },
  };

  // ---- 2.0 plane structure (P1/P2): adapter registry + plane registry (additive) ----
  const adapters = createAdapterRegistry();
  node.adapters = adapters;
  adapters.register(makeMcpAdapter(node));

  /** Engine deps scoped to one adapter (resourceMeta seam for non-MCP planes). */
  node.engineDepsFor = (adapterId) => {
    const a = adapters.get(adapterId);
    return {
      storage, bus, identity, authz,
      policy: policyEngine.resolvePolicySet(config.policyPacks),
      policyEngine,
      toolLookup: (mcpId, tool) => storage.q.toolGet.get(mcpId, tool || ''),
      monitor, srg, semantic, metrics, canary, config,
      cacheEnabled: config?.cache?.enabled !== false,
      resourceMeta: a && typeof a.metadata === 'function' ? (resource) => a.metadata(resource) : undefined,
    };
  };

  node.planes = {
    decision: { evaluate: (req, adapterId = 'mcp') => evaluateRequest(node.engineDepsFor(adapterId), req) },
    discovery: scanner,
    engines: { policyEngine, identity, authz, monitor, semantic, canary },
    adapters,
    response,
    srg,
    telemetry: metrics,
